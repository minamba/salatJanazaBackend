...
{
    public class GoogleTextImportStorageService : ITextImportStorageService
    {
        private readonly DriveService _drive;
        private readonly string _folderId;

        public GoogleTextImportStorageService(IConfiguration config)
        {
            _folderId = config["GoogleDrive:TextImportFolderId"]
                ?? throw new InvalidOperationException("GoogleDrive:TextImportFolderId manquant dans la config.");
            ...
            var refreshToken = config["GoogleDrive:RefreshToken"]
                ?? throw new InvalidOperationException("GoogleDrive:RefreshToken manquant dans la config.");
        ...
        public async Task<string> SaveAsync(string text, string fileName)
        {
            ...
            var request = _drive.Files.Create(fileMeta, stream, "text/plain");
            request.Fields = "id";
